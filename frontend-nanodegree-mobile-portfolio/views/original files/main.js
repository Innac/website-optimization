...
var resizePizzas = function(size) { 
  ...
  // randomPizzas[0] first pizza in the array, no? And they all have same size
  var randomPizzasCon =  document.getElementsByClassName("randomPizzaContainer");
  var dx = determineDx(randomPizzasCon[0], size);
  var newwidth = (randomPizzasCon[0].offsetWidth + dx) + 'px';

  function changePizzaSizes(size) {
    for (var i = 0; i < randomPizzasCon.length; i++) {
      randomPizzasCon[i].style.width = newwidth;
    }
  }
...
// Speeds up pizza loading process by ~0.1s
...
// TODO: create an array variable that has a reference to all of the pizzas that has the class name "mover"?
// TODO:  Do we necessarily need to calculate all of these phases for every pizza element if we know that there are only 5 unique phases?
...
function updatePositions() {
  ...
  for (var j = 0; j < items.length; j++) {
    items[j].style.left = items[j].basicLeft + 100 * phases[i%5] + 'px';
  }
...
document.addEventListener('DOMContentLoaded', function() {
  var c = 8;
  var s = 256;
  var m = window.screen.availHeight / s * 8;
  ...
  for (var i = 0; i < m; i++) {
    ...
    elem.basicLeft = (i % c) * s;
    elem.style.top = (Math.floor(i / c) * s) + 'px';
